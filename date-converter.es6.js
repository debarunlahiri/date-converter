/**
 * Date Converter (ES6 module, no dependencies).
 * Made by Debarun Lahiri
 * GitHub: https://github.com/debarunlahiri
 *
 * See README.md for supported inputs, tokens, timezone behavior, and examples.
 */
...
function fromZonedParts(p, timezone) {
  ...
  if (["year", "month", "day", "hour", "minute", "second"].some((key) => check[key] !== p[key])) {
...
function compileFormat(format) {
  ...
  let source = "^";
  let last = 0;
  format.replace(TOKENS, (token, index) => {
...
export function parseDate(value, options = {}) {
  ...
  const p = { year: 1970, month: 1, day: 1, hour: 0, minute: 0, second: 0, millisecond: 0 };
  let meridiem = null;
  let zone = null;
...
export function formatDate(date, pattern = "YYYY-MM-DD HH:mm:ss Z", timezone = "UTC") {
  assertTimezone(timezone);
  const p = getParts(date, timezone);
...
export function convertDate(value, options = {}) {
